 ...
 * config.d.ts so the clientSecret property can be annotated with
 * "@visibility secret" there.
 */
...
export class CasbinDBAdapterFactory {
  ...
  private async createAzureAdapter(
    ...
    return TypeORMAdapter.newAdapter({
      ...
      extra: {
        // Set max connection lifetime to 50 minutes (tokens expire after ~60 minutes)
        // This ensures connections are recycled before tokens expire
        idleTimeoutMillis: 50 * 60 * 1000,
      },
    });
  }
